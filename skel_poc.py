...
def _load_private_skel_bundle_auto():
    """V110.3.8 · Backblaze B2 Native API + stable runtime cache → /tmp.
...
def _plot_skel_fit(target_df, fit_rows, before_xyz, after_xyz, all_after=None, all_names=None):
    try:
        ...
        fig.update_layout(height=620,margin=dict(l=0,r=0,t=45,b=0),title="V110.3.8 · Frame semilla · XYZ objetivo vs SKEL",scene=dict(aspectmode="data"))
        st.plotly_chart(fig,use_container_width=True)
    except Exception as exc:
        st.caption(f"Visualización del fit no disponible: {exc}")

# V110.3.8 · COORDINATE FRAME REGISTRATION + BONE-VECTOR RETARGETING
...
def _empirical_select_dofs(model, rows, jacobian, max_dofs=_EMPIRICAL_MAX_DOF):
    """V110.3.8: selección empírica por cadena anatómica, usada como espacio reducido antes del registro de marcos y el ajuste por vectores óseos.
...
def _chain_bone_loss_torch(torch, model, world_joints, target_df, bone_pairs):
    names=[str(x) for x in getattr(model,'joints_name',[]) or []]; m={_simple_name(n):i for i,n in enumerate(names)}
...
def _refine_per_chain_ik(torch, model, pose, rotvec, trans, fixed_scale, target_df, rows, chain_map,
    ...
    # V110.3.8 · reconciliación anatómica global corta. Corrige residuos entre cadenas
...
# --- V110.3.8 · registro explícito de marco corporal y pérdidas por vectores óseos ---
_BONE_VECTOR_PAIRS = [
...
def _empirical_refine(torch,model,pose,rotvec,trans,fixed_scale,target_df,rows,selected,betas,zero_trans,
    ...
    for _ in range(int(iterations)):
        ...
        # V110.3.8: la dirección de los segmentos y el marco corporal pesan de forma explícita.
...
# V110.3.8 · SKEL FORWARD-KINEMATICS GROUND TRUTH
...
def _direct_group_loss_130(torch,model,world,target_df,rows,group):
    ...
    names=[str(x) for x in getattr(model,'joints_name',[]) or []]; nm={_simple_name(n):i for i,n in enumerate(names)}
    corr={'Hip':'pelvis','RHip':'righthip','RKnee':'rightknee','RAnkle':'rightankle','LHip':'lefthip','LKnee':'leftknee','LAnkle':'leftankle',
          'Neck':'neck','Head':'head','RShoulder':'rightshoulder','RElbow':'rightelbow','RWrist':'rightwrist','LShoulder':'leftshoulder','LElbow':'leftelbow','LWrist':'leftwrist'}
    pos=[]
    # V110.3.8 · profundidad segura: en reconstrucción monocular la Z es inferida,
...
# V110.3.8 · SKELAnatomicalAutomata
...
# V110.3.8 · calibración discreta del sistema de coordenadas SKEL ↔ V104/V107.
...
def _coordinate_system_calibration_134(model,joints_np,target_df,rows):
    ...
    # V110.3.8: el RMSE mostrado como 'neutro' se calcula también sobre EXACTAMENTE las mismas correspondencias (15) usadas aguas abajo.
...
# V110.3.8 · UNIFIED COORDINATE FRAME
...
# V110.3.8 · CALIBRACIÓN AUTOMÁTICA DE CADA DoF LOCAL SKEL
...
def _calibrate_local_dofs_135(torch,model,betas,zero_trans,axis_map,rotvec,scale,delta=0.08,unified_transform=None):
    """V110.3.8 · Kinematic-tree aware finite-difference calibration.
    ...
    return {'version':'110.3.8','delta_rad':float(delta),'rows':rows,'jac_world':jac_world,
...
def _fk_ground_truth_calibration_138(torch,model,betas,zero_trans,unified_transform,delta=0.06):
    ...
    chain_sets={
      'tronco':{'spine1','spine2','spine3','neck','left_collar','right_collar','left_shoulder','right_shoulder'},
      'cabeza':{'head'},
      'pierna_D':{'right_hip','right_knee','right_ankle','right_foot'},
      'pierna_I':{'left_hip','left_knee','left_ankle','left_foot'},
      'brazo_D':{'right_collar','right_shoulder','right_elbow','right_wrist'},
      'brazo_I':{'left_collar','left_shoulder','left_elbow','left_wrist'},
    }
    ...
    return {'version':'110.3.8','delta_rad':float(delta),'rows':rows,'jac_world':jac_world,'dependency_graph':graph,
...
def _monotonic_hierarchical_ik_138(torch,model,target_df,rows,betas,zero_trans,unified,pose_init,q_by_group,iterations_per_stage=55):
    ...
    return pose.cpu().numpy()[0].astype(np.float32), {'version':'110.3.8','history':history,'order':order,'accepted_stages':frozen,
...
def _automata_bounds_133(rv_seed,scale_seed,active_q,initial_pose=None,freeze_global=False):
    ...
    # Escala positiva en log-space; V110.3.8 puede congelarla como parte de T_SKEL→V104.
...
def _automata_eval_population_133(torch,model,X,active_q,betas,zero_trans,idx,tgt_np,target_df,axis_map=None,fixed_trans=None):
    ...
    names=[str(x) for x in getattr(model,'joints_name',[]) or []]; nm={_simple_name(nm0):i for i,nm0 in enumerate(names)}
    corr={'Hip':'pelvis','RHip':'righthip','RKnee':'rightknee','RAnkle':'rightankle','LHip':'lefthip','LKnee':'leftknee','LAnkle':'leftankle',
          'Neck':'neck','Head':'head','RShoulder':'rightshoulder','RElbow':'rightelbow','RWrist':'rightwrist','LShoulder':'leftshoulder','LElbow':'leftelbow','LWrist':'leftwrist'}
    tmap={r.joint:np.array([r.x,r.y,r.z],float) for r in target_df.itertuples()}
    bones=[]
    ...
    for i in range(n):
        ...
        # V110.3.8: con Unified Coordinate Frame la traslación también se congela.
...
def _fit_one_skel_frame(torch, model, target_df, max_iter=120, empirical_profile=None):
    ...
    with torch.no_grad():
        ...
        # V110.3.8: resolver primero la convención espacial (permutación/signos/handedness).
    ...
    # V110.3.8: ground truth por forward real de SKEL. No se decide la dependencia de q por su nombre.
    ...
    dof_calib={'version':'110.3.8','delta_rad':fk_gt.get('delta_rad',0.06),'rows':fk_gt.get('rows',[]),'jac_world':fk_gt.get('jac_world',{}),
    ...
    # V110.3.8: la búsqueda sólo cambia q. La similitud global queda INMUTABLE.
    ...
    return {'rows':rows,'joint_names':joint_names,'pose':pose.cpu().numpy()[0],'rotvec':rotvec.cpu().numpy(),
            ...
            'history':automata.get('history',[]),'automata':automata,'retargeting_mode':'fk_ground_truth_dependency_graph_monotonic_hierarchical_ik',
            ...
            'coordinate_frame_seed_rotvec':rv_seed.tolist(),'axis_map':axis_map.astype(float).tolist(),'coordinate_system_calibration':_json_safe_134(coord),'unified_coordinate_transform':_json_safe_134(unified),'unified_frame_invariance':_json_safe_134(frame_invariance),'unified_module_gate':_json_safe_134(unified_gate),'dof_local_calibration':_json_safe_134({k:v for k,v in dof_calib.items() if k!='jac_world'}),'fk_ground_truth':_json_safe_134({k:v for k,v in fk_gt.items() if k!='jac_world'}),'linear_dof_seed':_json_safe_134(linear_seed),'hierarchical_ik':_json_safe_134(hierarchical_ik),'direct_model_metadata':_direct_model_metadata_130(model)}
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None):
    ...
    for fi in range(int(start_index),len(frames)):
        ...
        with torch.no_grad():
            ...
            sequence.append({'frame':fi+1,'status':'ok','n_correspondences':len(rows),'rmse':rmse,'anatomical_audit':audit,
                'retargeting_mode':'fk_ground_truth_dependency_graph_monotonic_hierarchical_ik','active_q_names':[_SKEL_Q_NAMES_130[i] for i in _active_for_target_133(tdf)],
    ...
    return {'version':'110.3.8','retargeting_mode':'fk_ground_truth_dependency_graph_monotonic_hierarchical_ik','scale':float(scale.cpu()),
            'active_q_names':[_SKEL_Q_NAMES_130[i] for i in _active_for_target_133(tdf)], 'selected_q_indices':list(_DIRECT_ACTIVE_130),
            'joint_names':[str(x) for x in getattr(model,'joints_name',[])],
...
def _plot_skel_sequence_animation(seq):
    ...
    try:
        ...
        fig.update_layout(
            height=680,margin=dict(l=0,r=0,t=45,b=0),title="V110.3.8 · SKEL · marcha propagada temporalmente",
...
def _plot_simplified_xyz_animation(motion):
    ...
    try:
        ...
        fig.update_layout(height=600,margin=dict(l=0,r=0,t=45,b=0),title='V110.3.8 · Vídeo 3D simplificado V104/V107 · no calibrado',
...
# V110.3.8 · SKEL MESH WALKER: malla corporal real `skin_verts` sobre la secuencia temporal validada.
...
def _build_skel_skin_sequence(torch, model, seq, progress_cb=None):
    ...
    malla se aplica exactamente la escala/orientación/traslación guardada en la secuencia V110.3.8 activa.
    """
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        fig.update_layout(
            height=760,margin=dict(l=0,r=0,t=50,b=0),title="V110.3.8 · SKEL MESH WALKER · marcha anatómica 3D",
            scene=dict(aspectmode="data",xaxis=dict(range=[mins[0]-pad[0],maxs[0]+pad[0]]),yaxis=dict(range=[mins[1]-pad[1],maxs[1]+pad[1]]),zaxis=dict(range=[mins[2]-pad[2],maxs[2]+pad[2]])),
...
# V110.3.8 · CALIBRACIÓN EMPÍRICA DE LOS DOF SKEL
...
def _dof_calibration_json_bytes(df, profile, model):
    payload={'version':'110.3.8','method':'empirical central finite-difference around neutral pose',
             ...
             'warning':'Los nombres biomecanicos previos de q no se asumen. V110.3.8 usa el Jacobiano medido para seleccionar DOF observables.',
...
def render_skel_poc_panel(motion):
    ...
    st.markdown("**Modelo SKEL privado · V110.3.8 CPU + FK Ground Truth + Dependency Graph + Monotonic IK + Backblaze B2 Cache**")
    st.caption("V110.3.8 usa una caché runtime estable e independiente de la versión en `/tmp/physiosentinel_skel_b2_cache_v1_1`. Si el ZIP ya es válido no contacta Backblaze; sólo descarga en un MISS. Valida SHA/CRC y ambos PKL. Supabase no almacena el modelo y se mantiene fallback manual.")
    ...
    if not runtime:
        st.error("El bundle privado es válido, pero el runtime SKEL CPU no ha podido prepararse. V110.3.8 evita deliberadamente moderngl-window para mantener NumPy 2.x compatible con Pose2Sim/OpenSim.")
        ...
        # V110.3.8: auditar la estructura anatómica privada que gobierna el mapeo directo.
        private_meta={}
        try:
            import pickle
            ...
            st.error(f"El PKL SKEL existe, pero V110.3.8 no puede verificar su mapa anatómico interno: {type(meta_exc).__name__}: {meta_exc}")
            return
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                st.info("Puerta de runtime superada. V110.3.8 usa el mapa anatómico oficial q de SKEL. Ya no ejecuta calibración empírica ni selección global de DOF: el ajuste se limita a las articulaciones anatómicas correspondientes a cada cadena.")

                st.markdown("### V110.3.8 · Forward-Kinematics Ground Truth SKEL")
                ...
                with st.expander("Mapa anatómico q oficial utilizado por V110.3.8",expanded=False):
                    st.dataframe(pd.DataFrame(qrows),use_container_width=True,hide_index=True)
                profile=None

                st.markdown("### V110.3.8 · Frame semilla · Unified Frame → FK Ground Truth → Dependency Graph → Monotonic Hierarchical IK → Automata")
                ...
                try:
                    ...
                    st.markdown("#### V110.3.8 · Calibración automática del sistema de coordenadas")
                    ...
                    st.markdown("#### V110.3.8 · Unified Coordinate Frame · test de invariancia")
                    ...
                    st.markdown("#### V110.3.8 · Forward-Kinematics Ground Truth · 46 DoF")
                    ...
                    if _hik:
                        st.markdown('#### V110.3.8 · Monotonic Hierarchical IK · commit/rollback')
                        h1,h2=st.columns(2)
                    ...
                    st.info(f"Parámetros anatómicos calibrados V110.3.8: {len(selected_labels)} q con semántica SKEL oficial: " + ", ".join(selected_labels))
                    ...
                    export={
                        "version":"110.3.8","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                        "scale":fit["scale"],"translation":fit["trans"].tolist(),"global_rotation_axis_angle":fit["rotvec"].tolist(),
                        "pose_46":fit["pose"].tolist(),"rmse_xy":fit.get("rmse_xy"),"rmse_z":fit.get("rmse_z"),"automata":fit.get("automata",{}),"retargeting_mode":fit.get("retargeting_mode"),"bone_vector_audit":fit.get("bone_vector_audit",{}),"coordinate_frame_seed_rotvec":fit.get("coordinate_frame_seed_rotvec",[]),"axis_map":fit.get("axis_map",[]),"coordinate_system_calibration":fit.get("coordinate_system_calibration",{}),"unified_coordinate_transform":fit.get("unified_coordinate_transform",{}),"unified_frame_invariance":fit.get("unified_frame_invariance",{}),"unified_module_gate":fit.get("unified_module_gate",{}),"dof_local_calibration":fit.get("dof_local_calibration",{}),"fk_ground_truth":fit.get("fk_ground_truth",{}),"linear_dof_seed":fit.get("linear_dof_seed",{}),"hierarchical_ik":fit.get("hierarchical_ik",{}),"selected_q_indices":fit.get("selected_q",[]),"selected_q_labels":fit.get("active_q_names",[]),"chain_dof_map":fit.get("chain_dof_map",{}),"chain_mapping":fit.get("chain_mapping",[]),
                    ...
                    st.markdown("### V110.3.8 · Propagación temporal 1→75")
                    ...
                    if not frame_gate_ok:
                        st.warning("⛔ Puerta anatómica frame 1 NO superada. V110.3.8 no recomienda propagar 75 frames hasta corregir la pose semilla (RMSE XY <0.38 y auditoría ósea sin alertas).")
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        ...
                        if unsafe:
                            ...
                            st.success("✅ Control anatómico superado en todos los frames resueltos: sin plegados extremos detectados por la auditoría V110.3.8.")
                        ...
                        st.download_button("⬇️ Descargar secuencia SKEL V110.3.8 (JSON)",json.dumps(_json_safe_134(export_seq),ensure_ascii=False).encode("utf-8"),"V110_3_8_SKEL_sequence.json","application/json",use_container_width=True)
                        ...
                        st.markdown("### V110.3.8 · SKEL MESH WALKER")
                        st.caption("Convierte exclusivamente la secuencia anatómica V110.3.8 activa en `skin_verts`. Betas=0, escala fija y sólo los q anatómicos oficiales definidos para las cadenas observadas pueden apartarse de neutro.")
                        ...
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            st.download_button("⬇️ Descargar SKEL Mesh Walker V110.3.8 (NPZ)",_mesh_sequence_npz_bytes(mesh_seq),"V110_3_8_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True)
                            ...
                            st.markdown('### V110.3.8 · Validación sincronizada Side-by-Side')
                        ...
                        st.info("El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.3.8 habilitará la malla corporal animada.")
                except Exception as fit_exc:
                    st.error("SKEL funciona, pero V110.3.8 no ha podido completar el ajuste del frame semilla.")
